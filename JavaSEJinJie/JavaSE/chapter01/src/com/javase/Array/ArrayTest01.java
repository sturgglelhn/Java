...
Array
    ...
    5、数组当中如果存储的事“java对象”的话，实际上存储的是对象的“引用（内存地址）”,数组中不能直接存储java对象。
    6、数组一旦创建，在java中规定，长度不可变。（数组长度不可变）
    7、数组的分类：一维数组、二维数组、三维数组、多维数组...(一维数组使用较多)
    8、所有的数组对象都有length属性（java自带的），用来获取数组中元素的个数。
    9、数组中存储的元素类型必须统一，比如int类型数组只能存储int类型，Person类型只能存储Person类型。
    ...
    12、数组中每一个元素都是有下标的，下标从0开始，以1递增。最后一个元素的下标是：length-1，
        因为我们对数组中元素进行“存取”的时候，都需要通过下标来进行。
        ...
        优点：查询/查找/检索某个下标的元素时效率极高。可以说是查询效率最高的一个数据结构。
            为什么检索效率高？
                ...
                第三：知道第一个元素内存地址，知道每一个元素占用空间的大小，又知道下标，所以
                    通过一个数学表达式（偏移量）就可以计算出某个下标上元素的内存地址。直接通过内存地址
                    定位元素，所以数组检索效率是最高的。

                    数组中存储100个元素，或者存储100万个元素，在元素查询/检索方面，效率是相同的，
                    因为数组中元素查找的时候不会一个一个找，是通过数学表达式计算出来的。（算出
                    一个内存地址，直接定位的。）
        缺点：
            第一：由于为了保证数组中每个元素的内存地址连续，所以在数组上随机删除或者增加元素的时候，
                 效率较低，因为随机增删元素会涉及到后面元素统一向前或者向后唯一的操作。
            第二：数组不能存储大数据量，为什么？
...
public class ArrayTest01 {
    public static void main(String[] args) {
        ...
        System.out.println("最后一个元素 = " + a[a.length - 1]);  //等价 a[5]
        ...
        for(int i = 0; i<a.length; i++){
            System.out.println(a[i]); //i从0到5，是下标
        }
